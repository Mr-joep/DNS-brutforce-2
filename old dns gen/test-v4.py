...
def generate_subdomains(domain, max_length, progress_callback=None, progress_interval=0.1):
    characters = string.ascii_lowercase + string.digits
    ...
    for length in range(1, max_length + 1):
        for subset in itertools.product(characters, repeat=length):
            ...
            if progress_callback and progress - last_progress >= progress_interval:
                progress_callback(progress * 100)
                last_progress = progress

def save_subdomains_to_csv(subdomains, filename, chunk_size=1000000):
